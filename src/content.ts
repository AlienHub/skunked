...
interface UiDecision {
  action: "inject_overlay" | "inject_warning"
  data: RiskUiPayload
}

let badgeContainer: HTMLDivElement | null = null
let panelVisible = false
let domAnalysisRequested = false
...
function ensureBadge(payload: RiskUiPayload & { level: "block" | "warn" }) {
  ...
  const safeOfficialUrl = escapeHtml(payload.officialUrl)
  ...
  container.innerHTML = `
    ...
    ">
      ...
      <p style="margin:0 0 16px 0;color:#6b7280;font-size:13px;">
        SKUNKED 已为你拦截本次访问。建议通过官方入口重新打开。
      </p>
...
function applyUiDecision(ui: UiDecision | null | undefined) {
  if (!ui) return
  if (ui.action === "inject_overlay") {
    injectBlockOverlay(ui.data)
  }
  if (ui.action === "inject_warning") {
    injectWarningBar(ui.data)
  }
}

function requestDomAnalysis() {
  if (domAnalysisRequested) return
  domAnalysisRequested = true
  ...
  const run = () => {
    try {
      chrome.runtime.sendMessage(
        ...
        (response) => {
          applyUiDecision(response?.data?.ui)
        }
      )
    } catch {
      // Keep page usable if extraction fails.
    }
  }
...
function requestFastAnalysis() {
  chrome.runtime.sendMessage(
    {
      action: "analyze_url_fast",
      url: location.href
    },
    (response) => {
      if (!response?.success) return
      applyUiDecision(response.data?.ui)
      if (response.data?.needsDom) {
        requestDomAnalysis()
      }
    }
  )
